// Sedat's note to everyone : Please read all the comments in this file, however, remember that some of the comments in this file might be from earlier versions and may not apply to the most current version !!!!!!!!!
// Trak objects in the past frames untill you reach to the entered frame number. And then find the attributes for that object in that file. (for the expressions).
// check updateobjects for the disappearing objects again properly

// I am not sure if I was checking for the object being each place as well as checking its state conditions.. 
// i.e. I check if an object satisfies all the incoming states contition but I am not sure if I also check for that object being in that state (which is necessary as well).
// look into that in the future...
// Put the objects back into one of the initial places after they got into a final place..

// Note to sedat :  Using Vectors is not really the best way for speed. Therefore, consider using link list in the future. (once everything is done). :)
...
//void ReadVtkDataFile(vector<string> &variableNamesvect,string FileName, vector<float> &allvariables,vector<float> &xdimension,vector<float> &ydimension,vector<float> &zdimension,vector<float> &minvals,vector<float> &maxvals, int &x_dim,int &y_dim,int &z_dim,int &nnodes);
//void CreateNodeConn_Array(int** node_conn_array,int z_dim,int y_dim, int x_dim,int numberOfPointsInCell,int ncells);
//void CreateCoord_Array(vector<float> &allvariables,vector<float> &xdimension,vector<float> &ydimension,vector<float> &zdimension,float ** coord_array,float ** node_data,vector<float> &minvals,vector<float> &maxvals,int nspace,int nnodes,int ncells,int cellpoints,int nncomp);
//template <class T> extern int Segmentation(InpObject<T> *inPtr,int nspace, int nnodes, int ncells,int cellpoints,float *coord_array,int *node_conn_array, float *node_data,float thresh,std::string listfile, int timeFrame, char* mypolyfile, int smallest_vol,int precision,bool dovolren/*string volfile*/,int color_Method,int nncomp);
//void parseConfigFile(string Datapath,vector<string> &variableNamesvect, string Configfilename, int &InitialtimeStep, int &FinaltimeStep,float &deltaxval, float &deltayval, float &deltazval,int &SmallestObjVol, int &TimePrecision, int &TimeIncrement, float &thresh1, float &thresh2);
...
class pnTransition
{
  ...
  vector<Arc> incomingArcs; // incoming arcs are the input arcs defined in the Petrinet config file
  vector<Arc> outgoingArcs; // outgoing arcs are the output arcs defined in the Petrinet config file
...
class pnPlace
{
  ...
  vector<Arc> incomingArcs;  // these are the arcs from the conf file (inputarcs line)
  vector<Arc> outgoingArcs;  // these are the arcs from the conf file (outputarcs line)
...
{
  //this method tokenize the InputString based on the given parameter (lookfor) and puts all the tokens into OutputStringVector
  // example usage: 
  //               stringtokenizer( TokenizedStringVector,"((F0A5-F-2A6)*3/F-2A6) ; (F-1A6+F-2A6) ; (F-1A8+F-2A8)" , ";" )
  //TokenizedStringVector elements are: 
  //                            ((F0A5-F-2A6)*3/F-2A6) 
  //                            (F-1A6+F-2A6) 
  //                            (F-1A8+F-2A8)
...
void readPT_ConfFile0(string &filename, int &PNumber, int &transitionNumber,vector<Arc> &inputArcs, vector<Arc> &outputArcs, vector<int> &FinalPlaces,
...
void readPT_ConfFile2(string &filename, int &PNumber, int &transitionNumber,vector<Arc> &inputArcs, vector<Arc> &outputArcs, vector<int> &FinalPlaces,
                      vector<string> &transitionVector,vector<string> &placeVector,vector<int> &InitialPlaces,vector<string> &PetriNetVariables)
{
...
//
//
//void updateObjects(ThePetriNet &CurrentPetriNet, vector<stObject> &currentObjects, int currentFrame,vector<int> &initialPlaces,
//		 vector<int> &finalPlaces, int InitialtimeStep, vector<string> &trakTable,string trakTableFile)
//{
//  // from Sedat to Sedat: in the future seperate the updateobjets from this function... for a better computation...
...
//              EvaluateExpression(CurrentPetriNet,dummyplace, 0, returnValue, currentFrame,curObject + 1);
...
//              {
//                  //move the stobject info into the summary
...
//                          {
//                              // cout<< "Framenumber["<< currentFrame<<"] & actual ObjectID["<< objectIndexInCurrentObjects+1 <<" PastObjects are: ";
//                              for (int dumyCounter = 0; dumyCounter< dummyObjectListVect.size(); dumyCounter++)
//                              {
//                                  //cout<<" "<<dummyObjectListVect[dumyCounter];
//                                  vector<int> ObjectIndexes;
//                                  ObjectIndexes.clear();
//                                  for (int dumyCounter0 = 0; dumyCounter0 < currentPlaceObjects.size(); dumyCounter0++)
//                                  {
//                                      if ( currentPlaceObjects[dumyCounter0].RealObjectID == dummyObjectListVect[dumyCounter]  )
//                                      {
//                                          
//                                          cout<<" INSIDE THE IF STATEMENT !!!!!!!!!!!!!!!!!  currentframe: ["<< currentFrame<<"] ! OBjectID is: ["<< dummyObjectListVect[dumyCounter]<<"]" <<endl;
//                                          StObjSummary currObjsSummary1;
//                                          currObjsSummary1.PlaceID = dummyplace + 1;
//                                          currObjsSummary1.FrameNumber = currentFrame - 1;
//                                          currObjsSummary1.ObjectID.push_back( dummyObjectListVect[dumyCounter] );
//                                          curDummyObj.ObjectSummary.push_back( currObjsSummary1 );
//                                          ObjectIndexes.push_back(dumyCounter0);
//                                          currentPlaceObjects.erase( currentPlaceObjects.begin() + dumyCounter0); // remove the object and put it into the currentObject's history summary.
//                                          
//                                      }
//                                  }
//                                  // this for loop removes all the objects that are in dummyObjectListVect from the currentPlaceObjects
//                                  for (int dumyCounter0 = 0; dumyCounter0< ObjectIndexes.size(); dumyCounter0++)
//                                  {
//                                      currentPlaceObjects.erase( currentPlaceObjects.begin() + ObjectIndexes[dumyCounter0]);
//                                      for (int dumyCounter1 = 0; dumyCounter1< ObjectIndexes.size(); dumyCounter1++)
//                                      {
//                                          ObjectIndexes[dumyCounter1] = ObjectIndexes[dumyCounter1]-1;
//                                      }
//                                  }
//                                  
//                              }
...
//                              }
//                              double returnValue;
//                              EvaluateExpression(CurrentPetriNet,dummyplace, 0, returnValue, currentFrame,objectIndexInCurrentObjects + 1);
...
//                      EvaluateExpression(CurrentPetriNet,dummyplace, 0, returnValue, currentFrame, dumobj + 1);
        ...
        for(int dummytransition = 0; dummytransition < Tsize; dummytransition++ )
        {
            ...
            for(int dummyarc = 0; dummyarc <  CurrentPetriNet.CurrentTransitions[dummytransition].incomingArcs.size();dummyarc++)
            {
                ...
                for(int dumObjIndex = 0; dumObjIndex < CurrentPetriNet.CurrentPlaces[ dummyPlaceID - 1 ].currentObjectsInThisPlace.size();dumObjIndex++)
                {
                    ...
                    if (curDummyObj.activeObject == 1)
                    { 
                        cout<<"Inside evaluatePT5 2 before checkTransitionFired!!"<<endl;
                        transitionfires = checkTransitionFired( CurrentPetriNet, dummytransition+1, curDummyObj,currentFrame );
                        cout<<"Inside evaluatePT5 2 after checkTransitionFired!!"<<endl;
                    }
                    if ((transitionfires == 1 ) && (curDummyObj.activeObject == 1))
                    {
                        testcheck = 1;
...
// 	  if (dummyplacee != Place) // <=================== if you remove this if statement, then all the history will be written...
// 	  {
...
//(all events are 0 in the first frame for forward action detection) 1 = new born; 2 = dead;  3 =seperatation ; 4 = merge; 5 = continuation 
...
int main()
{
  ...
  string PNlabel = "/Users/liliu/eddy2.Conf";  
